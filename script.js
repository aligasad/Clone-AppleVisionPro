...
// onsrart mltb jaise hi trigger touch hoga to function chalega
gsap.to("#page>video",{
    scrollTrigger:{
        trigger:"#page>video",
        start:`2% top`,
...
// ye scroll karne pe scroll nahi hone dega phle video dikhayega fir csroll hone dega
gsap.to("#page",{
    scrollTrigger:{
        trigger:`#page`,
        start:`top top`,
...
gsap.to("#page-bottom",{
    scrollTrigger:{
        trigger:`#page-bottom`,
        start:`15% top`, //jaise hi 20% page scroll hoga opacity 0 ho jyegi
        end:`bottom top`,
        scroller:`#main`,
        scrub:.5,
    },
...
// TimeLine mtlb ise lahane se jbtk te top ko touch nahi krega first page scroll nahi hoga 
//kha se start krna hain triggerting
...
function canvas(){
    const canvas = document.querySelector("#page7>canvas"); // select krenge ke hmara canva kha rakha hai
...
window.addEventListener("resize", function () { //yw movement ke time pe image ki height width change krega
  canvas.width = window.innerWidth;
  canvas.height = window.innerHeight;
  render();
});
...
function files(index) {
 ...
 `;
  return data.split("\n")[index]; // is line ki madad se hm jo images string me rhti hia use image me convert krte hai
}

const frameCount = 198; // hm jitna images use kr rhe hai canva ke liye uska total no define krta h

const images = []; // ye blank array hai jisme saare images tag aayenge
const imageSeq = { //  frame 1 rhega jab bhi canvas chaleha
  frame: 1,
};

for (let i = 0; i < frameCount; i++) { 
  const img = new Image();  // is for loop se hme har baar image tag milega usme src dalega fir push ho jyega us blank array me
  img.src = files(i);    
  images.push(img);         
}
...
images[1].onload = render; // jab jab refresh hoga to first image rander hoga
...
function scaleImage(img, ctx) {
  ...
  var ratio = Math.min(hRatio, vRatio);  // jo bhi ratio bada hoga wo chalega (yha pe agar images landscape me hoti to hm "Math.max" lete kyuki uski width jyada hoti hai)
  var centerShift_x = (canvas.width - img.width * ratio) / 2;     // ye canvas ko center me rakhega
  var centerShift_y = (canvas.height - img.height * ratio) / 2;   // ye canvas ko center me rakhega
  ctx.clearRect(0, 0, canvas.width, canvas.height);  //  ye jb agli image load hogi to pichli image ko clear krega
  // ye har baar agli image ko load krega
  ctx.drawImage(  
    img,
    0,
    0,
